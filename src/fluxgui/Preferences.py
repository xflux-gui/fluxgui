...
class Preferences:
    ...
    def __init__(self, settings, xflux_controller):
        ...
        self.latsetting=self.connect_widget("entryLatitude",self.delete_event)
        self.lonsetting=self.connect_widget("entryLongitude",self.delete_event)
        self.zipsetting=self.connect_widget("entryZipcode",self.delete_event)
    ...
    def preview_click_event(self, widget, data=None):
        colsetting_temperature=self.temperatureKeys[self.colsetting.get_active()]
